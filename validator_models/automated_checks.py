...
def normalize_accents(text: str) -> str:
    """
    Remove accents and normalize Unicode for international name matching.
    Handles European, Asian, Arabic, and Cyrillic characters.
    José -> Jose, François -> Francois, Müller -> Muller
    """
    if not text:
        return text
    ...
    # First, apply Unicode normalization (NFKD for compatibility decomposition)
    # This handles most European accents
...
def _calculate_slug_similarity(slug1: str, slug2: str) -> float:
    """
    Calculate similarity between two LinkedIn profile slugs using edit distance.
    Handles numeric suffixes and formatting differences.
    ...
    s1 = slug1.lower().replace("-", "").replace("_", "").strip()
    s2 = slug2.lower().replace("-", "").replace("_", "").strip()
    
    # Exact match after normalization
    if s1 == s2:
        return 1.0
    
    # Check if one is substring of other (handles numeric suffixes like john-smith vs john-smith-123)
                    ...
                    if profile_slug:
                        if found_profile_urls:
                            ...
                            for result_slug in found_profile_urls:
                                ...
                                if profile_slug_norm == result_slug.lower().replace("-", "").replace("_", ""):
                                    exact_match = True
                                    break
                                
                                # Partial match (containment)
                                if (profile_slug_norm in result_slug.lower().replace("-", "").replace("_", "") or 
                                    result_slug.lower().replace("-", "").replace("_", "") in profile_slug_norm):
                                    partial_match = True
                                
                                # Fuzzy match using edit distance (≥85% similarity)
                                similarity = _calculate_slug_similarity(profile_slug, result_slug)
                                if similarity >= 0.85:
                                    fuzzy_match = True
                                    print(f"         ✅ FUZZY SLUG MATCH: '{profile_slug}' ≈ '{result_slug}' ({similarity:.0%} similar)")
                            if exact_match:
                                print(f"         ✅ URL MATCH: Profile '{profile_slug}' confirmed (exact)")
                                url_match_exact = True  # Strong identity proof!
                            elif fuzzy_match:
                                print(f"         ✅ URL MATCH: Profile '{profile_slug}' confirmed (fuzzy)")
                                url_match_exact = True  # Treat fuzzy match as strong proof
                            elif partial_match:
...
def locations_match_geopy(claimed: str, extracted: str, max_distance_km: float = 50) -> Tuple[bool, str]:
    ...
    # Allow 2 adjacent states (legitimate multi-location companies)
    ADJACENT_STATE_PAIRS = {
        frozenset(['new york', 'new jersey']),
        frozenset(['new york', 'connecticut']),
        frozenset(['california', 'nevada']),
        frozenset(['texas', 'louisiana']),
        frozenset(['illinois', 'indiana']),
        frozenset(['washington', 'oregon']),
        frozenset(['massachusetts', 'rhode island']),
        frozenset(['virginia', 'maryland']),
        frozenset(['pennsylvania', 'new jersey']),
        frozenset(['florida', 'georgia']),
        # Add abbreviation pairs
        frozenset(['ny', 'nj']),
        frozenset(['ca', 'nv']),
        frozenset(['tx', 'la']),
        frozenset(['il', 'in']),
    ...
    elif len(states_found) == 2:
        # Check if they're adjacent states (legitimate)
        state_pair = frozenset(states_found)
        if state_pair not in ADJACENT_STATE_PAIRS:
            # Check if it's a context like "offices in X and Y" (legitimate)
            if "office" in claimed_lower or "location" in claimed_lower or "headquarter" in claimed_lower:
                print(f"   ℹ️  Two states detected but appears to be multi-location company: {states_found}")
            else:
                return False, f"ANTI-GAMING: Non-adjacent states detected (likely gaming): {states_found}"
...
# Extended role taxonomy for better semantic matching
ROLE_EQUIVALENCIES = {
    ...
    # NEW: Sales roles
    "sales director": ["director of sales", "sales director", "head of sales", "vp of sales", "vp sales"],
    "sales manager": ["sales manager", "manager of sales", "regional sales manager", "area sales manager"],
    "account executive": ["account executive", "ae", "sales executive", "business development executive"],
    # NEW: Marketing roles
    "marketing director": ["director of marketing", "marketing director", "head of marketing", "vp of marketing", "vp marketing"],
    "marketing manager": ["marketing manager", "manager of marketing", "brand manager", "product marketing manager"],
    # NEW: Operations roles
    "operations director": ["director of operations", "operations director", "head of operations", "vp of operations", "vp operations"],
    "operations manager": ["operations manager", "manager of operations", "general manager", "gm"],
    # NEW: Technology roles
    "engineering manager": ["engineering manager", "manager of engineering", "development manager", "software engineering manager"],
    "tech lead": ["tech lead", "technical lead", "lead engineer", "lead developer"],
    # NEW: Finance roles
    "finance director": ["director of finance", "finance director", "head of finance", "vp of finance", "vp finance"],
    "controller": ["controller", "financial controller", "comptroller"],
    # NEW: HR roles
    "hr director": ["director of hr", "hr director", "head of hr", "vp of hr", "vp human resources", "director of human resources"],
    "hr manager": ["hr manager", "manager of hr", "human resources manager", "people manager"],
}
...
def fuzzy_match_role(claimed_role: str, extracted_role: str) -> Tuple[bool, float, str]:
    ...
    if claimed_words and extracted_words:
        ...
        # Lower threshold for high-confidence extractions (from verified LinkedIn profiles)
        # or when roles share department/level indicators
        threshold = 0.6
        has_level_match = any(level in claimed_role.lower() and level in extracted_role.lower() 
                            for level in ["senior", "junior", "lead", "principal", "staff", "associate"])
        has_dept_match = (claimed_dept == extracted_dept) if (claimed_dept and extracted_dept) else False
        if has_level_match or has_dept_match:
            threshold = 0.5  # Lower threshold when level or department matches
            
        if jaccard >= threshold:
            confidence_note = f" (lowered threshold due to {'level' if has_level_match else 'department'} match)" if threshold < 0.6 else ""
            return True, jaccard, f"Word overlap: {jaccard:.0%} - common words: {intersection}{confidence_note}"
...
def fuzzy_pre_verification_stage5(
    ...
    # INDUSTRY FUZZY MATCHING (NEW)
    if industry_search_results and claimed_industry:
        print(f"   🔍 DEBUG INDUSTRY: Attempting fuzzy match before LLM...")
        
        # Industry taxonomy for fuzzy matching
        INDUSTRY_EQUIVALENCIES = {
            "technology": ["technology", "tech", "software", "saas", "it", "information technology", "software development"],
            "financial services": ["financial services", "finance", "fintech", "banking", "investment", "venture capital", "private equity"],
            "healthcare": ["healthcare", "health", "medical", "biotech", "pharma", "pharmaceutical", "medical devices"],
            "retail": ["retail", "e-commerce", "ecommerce", "consumer", "consumer products", "consumer goods"],
            "hospitality": ["hospitality", "food & beverage", "food and beverage", "restaurant", "hotel", "bar", "catering"],
            ...
            "telecommunications": ["telecommunications", "telecom", "wireless", "networking"],
        ...
        claimed_industry_lower = claimed_industry.lower().strip()
        
        # Check if claimed industry matches any taxonomy category
        matched_category = None
        for category, variants in INDUSTRY_EQUIVALENCIES.items():
            if any(variant in claimed_industry_lower for variant in variants):
                matched_category = category
                break
        
        if matched_category:
            # Search for industry keywords in search results
            industry_text = ""
            for r in industry_search_results[:5]:
                title = r.get("title", "")
                snippet = r.get("snippet", r.get("body", ""))
                industry_text += f"{title} {snippet} ".lower()
            
            # Check if any variant of the matched category appears in search results
            category_variants = INDUSTRY_EQUIVALENCIES[matched_category]
            found_match = any(variant in industry_text for variant in category_variants)
            
            if found_match:
                print(f"   ✅ FUZZY INDUSTRY MATCH: '{claimed_industry}' ≈ '{matched_category}' (found in search results)")
                result["industry_verified"] = True
                result["industry_extracted"] = matched_category
                result["industry_confidence"] = 0.85
                result["industry_reason"] = f"Fuzzy match via taxonomy: {matched_category}"
                # Don't need LLM for industry
                if "industry" in result.get("needs_llm", []):
                    result["needs_llm"].remove("industry")
            else:
                print(f"   ⚠️ FUZZY INDUSTRY: Category '{matched_category}' not found in search results, sending to LLM")
        else:
            print(f"   ℹ️  FUZZY INDUSTRY: No taxonomy match for '{claimed_industry}', sending to LLM")
    return result
